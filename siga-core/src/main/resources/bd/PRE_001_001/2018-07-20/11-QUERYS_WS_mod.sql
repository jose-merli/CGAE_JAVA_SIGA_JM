...
TABLESPACE TS_SIGA_CMN_LOB LOGGING;

COMMENT ON COLUMN cmn_datos_xml.id_datos_xml IS
    'primary key generada por la secuencia: SEQ_CMN_DATOS_XML';

ALTER TABLE cmn_datos_xml DROP CONSTRAINT pk_cmn_datos_xml;
ALTER TABLE cmn_datos_xml ADD CONSTRAINT pk_cmn_datos_xml PRIMARY KEY ( id_datos_xml ) USING INDEX TABLESPACE TS_SIGA_CMN_IDX;
...
CREATE UNIQUE INDEX pk_cargas_ws_idx ON
    cargas_ws ( id_carga ASC )
        LOGGING TABLESPACE TS_SIGA_CMN_IDX;
...
CREATE UNIQUE INDEX pk_cargas_ws_pag_idx ON
    cargas_ws_pagina ( id_ws_pagina ASC ) TABLESPACE TS_SIGA_CMN_IDX LOGGING;
...
TABLESPACE TS_SIGA_MGN LOGGING;
...
CREATE INDEX cfg_param_colegio_idx ON
    cfg_param_colegios ( id_param_colegio ASC )
        TABLESPACE TS_SIGA_MGN_IDX LOGGING;
