...
public class ModModeloPlantillaDocumentoExtendsSqlProvider {
	...
	public String selectInformesGenerarConDest(Long idModeloComunicacion, String idLenguaje){
		...
		sql.INNER_JOIN("con_consulta ON con_consulta.idconsulta=MOD_PLANTILLADOC_CONSULTA.Idconsulta AND con_consulta.idinstitucion = MOD_PLANTILLADOC_CONSULTA.Idinstitucion_consulta");
		sql.WHERE("modPlantilla.IDMODELOCOMUNICACION = " + idModeloComunicacion + " AND modPlantilla.FECHABAJA IS NULL and con_consulta.idobjetivo = 1 AND mod_plantilladoc_consulta.fechabaja IS NULL"
				+ "      AND con_consulta.fechabaja IS NULL");
		sql.GROUP_BY("modPlantilla.IDINFORME,modPlantilla.NOMBREFICHEROSALIDA, modPlantilla.FORMATOSALIDA, modPlantilla.IDPLANTILLADOCUMENTO");
