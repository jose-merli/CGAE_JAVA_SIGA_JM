...
//		}
	...
	public String getCalendariosProgramadosSigaClassique(CalendariosProgDatosEntradaItem calendarioItem, String idInstitucion) {
		...
		sql.SELECT("DECODE((" + sqlGenerado + "), 0, 'No', 'Si') AS GENERADO");
...
//			}
		if (calendarioItem.getFechaCalendarioDesde() != null && calendarioItem.getFechaCalendarioDesde() != "") {
			sql.WHERE("trunc(PC.FECHACALFIN) >= " + "TO_DATE('" + calendarioItem.getFechaCalendarioDesde() + "','DD/MM/YYYY')");
		}
		if (calendarioItem.getFechaCalendarioHasta() != null && calendarioItem.getFechaCalendarioHasta() != "") {
			sql.WHERE("trunc(PC.FECHACALINICIO) <= " + "TO_DATE('" + calendarioItem.getFechaCalendarioHasta()+ "','DD/MM/YYYY')");	
		}
		if (calendarioItem.getIdTurno() != null && calendarioItem.getIdTurno() != "") {
...
//				 ESTADO IN (programado ,reprogramado ) ORDER BY ORDEN   ) WHERE  ROWNUM=1
	...
	subquery.WHERE("PC.IDINSTITUCION = GT.IDINSTITUCION AND PC.IDGUARDIA = GT.IDGUARDIA AND PC.IDTURNO = GT.IDTURNO AND PC.IDINSTITUCION = " + idInstitucion + " AND PC.IDPROGCALENDARIO= " + calendarioItem.getIdCalendarioProgramado());
	//+ " AND" + "				 ESTADO IN (1 , 5)");
