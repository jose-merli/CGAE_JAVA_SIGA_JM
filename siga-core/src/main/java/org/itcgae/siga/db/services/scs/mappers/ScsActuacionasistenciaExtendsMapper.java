...
public interface ScsActuacionasistenciaExtendsMapper extends ScsActuacionasistenciaMapper {
    ...
    List<ComboItem> comboTipoActuacion(Short idInstitucion, String idTipoAsistencia, Integer idLenguaje);
    
    @SelectProvider(type= ScsActuacionasistenciaSqlExtendsProvider.class, method="comboTipoActuacionFicha")
    @Results({
            @Result(column="ID", property="value", jdbcType= JdbcType.VARCHAR),
            @Result(column="DESCRIPCION", property="label", jdbcType=JdbcType.VARCHAR),
    })
    List<ComboItem> comboTipoActuacionFicha(Short idInstitucion, ScsAsistencia scsAsistencia, String idTipoActuacion,
			Integer idLenguaje);
