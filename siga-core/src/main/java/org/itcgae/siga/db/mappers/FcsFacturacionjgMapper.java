...
public interface FcsFacturacionjgMapper {
	...
	@Update({ "update FCS_FACTURACIONJG", "set FECHADESDE = #{fechadesde,jdbcType=TIMESTAMP}, ",
			...
			"REGULARIZACION = #{regularizacion,jdbcType=VARCHAR}, FECHAMODIFICACION = #{fechamodificacion,jdbcType=TIMESTAMP}, ",
			"USUMODIFICACION = #{usumodificacion,jdbcType=DECIMAL}, IDFACTURACION_REGULARIZA = #{idfacturacionRegulariza,jdbcType=DECIMAL}, ",
			"NOMBREFISICO = #{nombrefisico,jdbcType=VARCHAR}, IDECOMCOLA = #{idecomcola,jdbcType=DECIMAL}, VISIBLE = #{visible,jdbcType=VARCHAR}, ",
			"IDPARTIDAPRESUPUESTARIA = #{idpartidapresupuestaria,jdbcType=DECIMAL}, ARCHIVADA = #{archivada,jdbcType=DECIMAL}",
			"where IDINSTITUCION = #{idinstitucion,jdbcType=DECIMAL} and IDFACTURACION = #{idfacturacion,jdbcType=DECIMAL}" })
