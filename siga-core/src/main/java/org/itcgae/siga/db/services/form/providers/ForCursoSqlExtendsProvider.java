...
public class ForCursoSqlExtendsProvider extends ForCursoSqlProvider {
	...
	public String selectCursos(Short idInstitucion, CursoItem cursoItem, String idLenguaje) {
		...
		sql3.SELECT("pys.DESCRIPCION");
		sql3.FROM("PYS_PRECIOSSERVICIOS pys");
		sql3.INNER_JOIN("PYS_PERIODICIDAD per on per.IDPERIODICIDAD = pys.IDPERIODICIDAD where pys.idInstitucion = '" + idInstitucion + "' GROUP BY pys.DESCRIPCION");
		sql.LEFT_OUTER_JOIN("(" + sql3 + ")  MINPRECIO ON MINPRECIO.descripcion = CURSO.CODIGOCURSO");
		
		SQL sql4 = new SQL();
		sql4.SELECT("MAX(pys.valor) as PRECIOHASTA");
		sql4.SELECT("pys.DESCRIPCION");
		sql4.FROM("PYS_PRECIOSSERVICIOS pys");
		sql4.INNER_JOIN("PYS_PERIODICIDAD per on per.IDPERIODICIDAD = pys.IDPERIODICIDAD where pys.idInstitucion = '" + idInstitucion + "' GROUP BY pys.DESCRIPCION");
