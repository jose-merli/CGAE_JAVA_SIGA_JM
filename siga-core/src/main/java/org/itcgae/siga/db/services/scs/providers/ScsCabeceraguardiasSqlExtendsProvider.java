...
public class ScsCabeceraguardiasSqlExtendsProvider extends ScsCabeceraguardiasSqlProvider{
	...
	public String busquedaGuardiasColegiado(GuardiasItem guardiaItem, String idInstitucion, Integer tamMax) {
		...
		sql.SELECT("guard.validado,"
				...
				+ "(SELECT count(*) FROM SCS_GUARDIASCOLEGIADO sg WHERE sg.IDINSTITUCION = guard.idinstitucion\r\n"
				+ "		AND sg.IDTURNO = guard.idturno AND sg.IDGUARDIA = guard.idguardia AND sg.IDPERSONA = guard.idpersona\r\n"
				+ "		AND sg.FECHAINICIO = guard.fechainicio) AS dias, "
				+ "    ("
				+ "        SELECT"
				+ "            t.nombre"
				+ "        FROM"
