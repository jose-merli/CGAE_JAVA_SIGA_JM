...
public class CenColegiadoSqlExtendsProvider extends CenColegiadoSqlProvider {
	...
	public String selectColegiados(Short idInstitucion, ColegiadoItem colegiadoItem) {
		...
		sql.SELECT("concat( decode(col.situacionresidente,0,'No', 'Sí')  || ' / ',decode(col.comunitario,0,'No', 'Sí')) as residenteInscrito");
		...
		sql.INNER_JOIN(
				"cen_estadocolegial estcol on (colest.idestado = estcol.idestado)");
		sql.INNER_JOIN(
				"gen_recursos_catalogos cat on (estcol.descripcion = cat.idrecurso and cat.idlenguaje = '1')");
		sql.INNER_JOIN("cen_direcciones dir on (cli.idpersona = dir.idpersona and cli.idinstitucion = dir.idinstitucion and inst.idinstitucion = dir.idinstitucion and dir.fechabaja is null)");
		...
		if (colegiadoItem.getFechaNacimiento() != null) {
			sql.WHERE("TO_DATE(PER.FECHANACIMIENTO,'DD/MM/RRRR') = TO_DATE('" + colegiadoItem.getFechaNacimiento()
					+ "','DD/MM/YYYY')");
		}
		
//		if (colegiadoItem.getFechaIncorporacionDesde() != null) {
//			sql.WHERE("TO_DATE(col.fechaincorporacion,'DD/MM/RRRR') >= TO_DATE('" + colegiadoItem.getFechaIncorporacionDesde()
//					+ "','DD/MM/YYYY')");
//		}
//		
//		if(colegiadoItem.getFechaIncorporacionHasta() != null) {
//			sql.WHERE("TO_DATE(col.fechaincorporacion,'DD/MM/RRRR') <= TO_DATE('" + colegiadoItem.getFechaIncorporacionHasta() + "','DD/MM/YYYY')");
//		}
