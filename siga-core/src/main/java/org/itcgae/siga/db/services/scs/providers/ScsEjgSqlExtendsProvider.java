...
//				sql.WHERE ("EJG.NUMEJG ='09039'");
    ...
    public String getAsuntoTipoEjg(AsuntosClaveJusticiableItem asuntoClave, String idLenguaje) {
        ...
        sql.SELECT("concat('X' || EXPEDIENTE.anioexpediente || '/', EXPEDIENTE.numeroexpediente) AS EXPEDIENTEINSOSTENIBILIDAD");
	...
	public String getDictamen(EjgItem ejgItem, String idInstitucion, String idLenguaje) {
		...
		sql.SELECT("ejg.fechadictamen," + 
				" ejg.dictamen as observaciones," + 
				" recdictamen.descripcion as dictamen," + 
				" ejg.idtipodictamenejg as iddictamen," + 
				" recfundamento.descripcion as fundamento," + 
				" ejg.idfundamentocalif as idfundamento");
		
		sql.FROM("scs_ejg ejg");
		sql.LEFT_OUTER_JOIN("scs_tipodictamenejg tipodictamen on (tipodictamen.idtipodictamenejg=ejg.idtipodictamenejg and tipodictamen.idinstitucion=ejg.idinstitucion)");
		sql.LEFT_OUTER_JOIN("gen_recursos_catalogos recdictamen on (tipodictamen.descripcion=recdictamen.idrecurso and recdictamen.idlenguaje = '" + idLenguaje + "')");
		sql.LEFT_OUTER_JOIN("scs_tipofundamentocalif tipofundamento on (tipofundamento.idtipodictamenejg=ejg.idtipodictamenejg and tipofundamento.idfundamentocalif=ejg.idfundamentocalif and tipofundamento.idinstitucion=ejg.idinstitucion)");
		sql.LEFT_OUTER_JOIN("gen_recursos_catalogos recfundamento on (tipofundamento.descripcion=recfundamento.idrecurso and recfundamento.idlenguaje = '" + idLenguaje + "')");
