...
public class FcsPagosjgSqlExtendsProvider extends FcsPagosjgSqlProvider {
	...
	public String getConceptosPago(String idPago, Short idInstitucion, String lenguaje) {
		...
		sql.SELECT("(CASE\r\n"
				+ "		WHEN FPGH.IDHITOGENERAL = 10 THEN NVL(DECODE(FF.IMPORTEOFICIO, 0, 0, ROUND(FP.IMPORTEOFICIO * 100 / FF.IMPORTEOFICIO , 2)), 0)\r\n"
				+ "		WHEN FPGH.IDHITOGENERAL = 20 THEN NVL(DECODE(FF.IMPORTEGUARDIA, 0, 0, ROUND(FP.IMPORTEGUARDIA * 100 / FF.IMPORTEGUARDIA , 2)), 0)\r\n"
				+ "		WHEN FPGH.IDHITOGENERAL = 30 THEN NVL(DECODE(FF.IMPORTEEJG, 0, 0, ROUND(FP.IMPORTEEJG * 100 / FF.IMPORTEEJG , 2)), 0)\r\n"
				+ "		WHEN FPGH.IDHITOGENERAL = 40 THEN NVL(DECODE(FF.IMPORTESOJ, 0, 0, ROUND(FP.IMPORTESOJ * 100 / FF.IMPORTESOJ , 2)), 0)\r\n"
				+ "	END) PORCENTAJEPAGADO");
