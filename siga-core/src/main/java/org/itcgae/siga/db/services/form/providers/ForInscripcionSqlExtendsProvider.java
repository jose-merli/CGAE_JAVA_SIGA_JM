...
public class ForInscripcionSqlExtendsProvider extends ForInscripcionSqlProvider {
	...
	public String selectInscripciones(InscripcionItem inscripcionItem, String idLenguaje) {
		...
		sql3.SELECT("SERVICIO.DESCRIPCION");
		...
		sql3.INNER_JOIN("PYS_PERIODICIDAD per on per.IDPERIODICIDAD = pys.IDPERIODICIDAD where pys.idInstitucion = '" + inscripcionItem.getIdInstitucion() + "' GROUP BY SERVICIO.DESCRIPCION");
		sql.LEFT_OUTER_JOIN("(" + sql3 + ")  MINPRECIO ON MINPRECIO.DESCRIPCION = CURSO.CODIGOCURSO");
		
		SQL sql4 = new SQL();
		sql4.SELECT("MAX(pys.valor) as PRECIOHASTA");
		sql4.SELECT("SERVICIO.DESCRIPCION");
		...
		sql4.INNER_JOIN("PYS_PRECIOSSERVICIOS PYS ON PYS.IDSERVICIO = SERVICIO.IDSERVICIO AND SERVICIO.IDTIPOSERVICIOS = PYS.IDTIPOSERVICIOS  "
				+ "AND PYS.IDSERVICIOSINSTITUCION = SERVICIOINST.IDSERVICIOSINSTITUCION");
		sql4.INNER_JOIN("PYS_PERIODICIDAD per on per.IDPERIODICIDAD = pys.IDPERIODICIDAD where pys.idInstitucion = '" + inscripcionItem.getIdInstitucion() + "' GROUP BY SERVICIO.DESCRIPCION");
		sql.LEFT_OUTER_JOIN("(" + sql4 + ")  MAXPRECIO ON MAXPRECIO.DESCRIPCION = CURSO.CODIGOCURSO");
