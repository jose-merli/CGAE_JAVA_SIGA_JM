...
public class FacHistoricofacturaExtendsSqlProvider extends FacHistoricofacturaSqlProvider {
	...
	public String getEstadosPagos(String idFactura, String idInstitucion, String idLenguaje) {
		...
		sql.SELECT("FH.FECHAMODIFICACION, F_SIGA_GETRECURSO(FT.NOMBRE," + idLenguaje + ") ACCION,"
				...
				+ "CASE WHEN FT.IDTIPOACCION = 2 OR ((FT.IDTIPOACCION = 8 OR FT.IDTIPOACCION = 9) AND FH.COMISIONIDFACTURA IS NOT NULL) THEN '1' END ENLACEFACTURA," + "FF.NUMEROFACTURA,"
				...
				+ "CASE WHEN (FT.IDTIPOACCION = 8 OR FT.IDTIPOACCION = 9) AND FH.COMISIONIDFACTURA IS NULL THEN '1' END ENLACEABONO,"
				+ "FA.NUMEROABONO,"
				+ "CASE WHEN FT.IDTIPOACCION = 8 THEN FA.IDABONO WHEN FT.IDTIPOACCION = 9 THEN FA.IDABONO END IDABONO,"
				+ "CASE WHEN FT.IDTIPOACCION = 8 OR FT.IDTIPOACCION = 9 THEN FH.COMISIONIDFACTURA ELSE NULL END COMISIONIDFACTURA,"
				+ "CASE WHEN FT.IDTIPOACCION = 8 OR FT.IDTIPOACCION = 9 THEN (SELECT CF.NUMEROFACTURA FROM FAC_FACTURA CF WHERE CF.IDINSTITUCION = FH.IDINSTITUCION AND CF.IDFACTURA = FH.COMISIONIDFACTURA) ELSE NULL END COMISIONFACTURA");
