...
public class FacHistoricofacturaExtendsSqlProvider extends FacHistoricofacturaSqlProvider {
	...
	public String getEstadosPagos(String idFactura, String idInstitucion, String idLenguaje) {
		...
		forward.SELECT_DISTINCT("FH.FECHAMODIFICACION");
		...
		forward.SELECT_DISTINCT("CASE WHEN FT.IDTIPOACCION = 8 OR FT.IDTIPOACCION = 9 THEN (SELECT CF.NUMEROFACTURA FROM FAC_FACTURA CF WHERE CF.IDINSTITUCION = FH.IDINSTITUCION AND CF.IDFACTURA = FH.COMISIONIDFACTURA) ELSE NULL END COMISIONFACTURA");
		...
		/*
		sql.SELECT("FH.FECHAMODIFICACION");
		sql.SELECT("INITCAP(F_SIGA_GETRECURSO(FT.NOMBRE," + idLenguaje + ")) ACCION");
		sql.SELECT("FT.IDTIPOACCION, FH.ESTADO IDESTADO");
		sql.SELECT("INITCAP(GR.DESCRIPCION) ESTADO");
		sql.SELECT("CC.IBAN");
		sql.SELECT("FH.IMPTOTALPAGADO");
		sql.SELECT("FH.IMPTOTALPORPAGAR");
		sql.SELECT("CASE WHEN FT.IDTIPOACCION = 10 THEN TO_CHAR(FA.IDPAGOSJG) END IDSJCS");
		sql.SELECT("FF.NUMEROFACTURA");
		sql.SELECT("CASE WHEN FT.IDTIPOACCION = 2 OR ((FT.IDTIPOACCION = 8 OR FT.IDTIPOACCION = 9) AND FH.COMISIONIDFACTURA IS NOT NULL) THEN '1' END ENLACEFACTURA");
		sql.SELECT("CASE WHEN FT.IDTIPOACCION = 2 THEN FF.IDFACTURA END IDFACTURA");
		sql.SELECT("cASE WHEN FT.IDTIPOACCION = 5 THEN TO_CHAR(FH.IDDISQUETECARGOS) END IDCARGOS");
		sql.SELECT("CASE WHEN FT.IDTIPOACCION = 6 THEN TO_CHAR(IDDISQUETEDEVOLUCIONES) END IDDEVOLUCIONES");
		sql.SELECT("CASE WHEN (FT.IDTIPOACCION = 8 OR FT.IDTIPOACCION = 9) AND FH.COMISIONIDFACTURA IS NULL THEN '1' END ENLACEABONO");
		sql.SELECT("FA.NUMEROABONO");
		sql.SELECT("CASE WHEN FT.IDTIPOACCION = 8 THEN FA.IDABONO WHEN FT.IDTIPOACCION = 9 THEN FA.IDABONO END IDABONO");
		sql.SELECT("CASE WHEN FT.IDTIPOACCION = 8 OR FT.IDTIPOACCION = 9 THEN FH.COMISIONIDFACTURA ELSE NULL END COMISIONIDFACTURA");
		sql.SELECT("CASE WHEN FT.IDTIPOACCION = 8 OR FT.IDTIPOACCION = 9 THEN (SELECT CF.NUMEROFACTURA FROM FAC_FACTURA CF WHERE CF.IDINSTITUCION = FH.IDINSTITUCION AND CF.IDFACTURA = FH.COMISIONIDFACTURA) ELSE NULL END COMISIONFACTURA");
