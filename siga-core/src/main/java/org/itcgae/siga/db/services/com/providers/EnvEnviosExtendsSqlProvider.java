...
//			fechaCreacion2 += " 23:59:59";
	...
	public String selectEnviosComunicacion(Short idInstitucion, String idLenguaje,EnviosMasivosSearch filtros){
		...
		sql.FROM("ENV_ENVIOS ENVIO");
//		JOIN ENV_PLANTILLASENVIOS PLANTILLA ON PLANTILLA.IDINSTITUCION = '2005' AND PLANTILLA.IDPLANTILLAENVIOS = ENVIO.IDPLANTILLAENVIOS AND PLANTILLA.IDTIPOENVIOS = ENVIO.IDTIPOENVIOS

		sql.LEFT_OUTER_JOIN("ENV_CAMPOSENVIOS CAMPOSENVIOSASUNTO ON (ENVIO.IDENVIO = CAMPOSENVIOSASUNTO.IDENVIO AND CAMPOSENVIOSASUNTO.IDINSTITUCION = ENVIO.IDINSTITUCION AND CAMPOSENVIOSASUNTO.IDCAMPO =1)");
		sql.LEFT_OUTER_JOIN("ENV_CAMPOSENVIOS CAMPOSENVIOSCUERPO ON (ENVIO.IDENVIO = CAMPOSENVIOSCUERPO.IDENVIO AND CAMPOSENVIOSCUERPO.IDINSTITUCION = ENVIO.IDINSTITUCION AND CAMPOSENVIOSCUERPO.IDCAMPO =2)");
		sql.JOIN("ENV_PLANTILLASENVIOS PLANTILLA ON PLANTILLA.IDINSTITUCION = '" + idInstitucion + "' AND PLANTILLA.IDPLANTILLAENVIOS = ENVIO.IDPLANTILLAENVIOS AND PLANTILLA.IDTIPOENVIOS = ENVIO.IDTIPOENVIOS");
