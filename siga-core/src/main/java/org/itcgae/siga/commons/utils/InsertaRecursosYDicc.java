...
public class InsertaRecursosYDicc {
	...
	try {
	    ...
	    while (enumer.hasMoreElements()) {
	    	...
	    	if (valor!=null) {
	    	    if (!existeIdRecurso(clave) && !existeEnDiccionario(clave)) {
		    		//trace.append("\n delete GEN_RECURSOS where IDRECURSO='"+clave+"';");
	    	    	insertGenRecursos(trace, clave, valor);
	    	    	...
	    	    	if (!existeIdRecurso(clave)) {
	    	    		...
	    	    		InputStreamReader isr=new InputStreamReader(System.in);
		    	        BufferedReader br=new BufferedReader(isr);
		    	        System.out.println("Se ha encontrado la clave "+clave+" en GEN_RECURSOS");
		    	        System.out.println("Es una modificación de recurso? (s/n)");
		    		    String texto = br.readLine();
		    	        if (texto.equals("s")) {
				    		if (!existeEnDiccionario(clave)) {
				    			updateGenRecursos(trace, clave, valor);
					    		insertGenDiccionario(trace, clave, valor);
				    		}else {
		    	            ...
		    	            System.out.println("ATENCIÓN!!! Estas usando un idrecurso en GEN_RECURSO que ya existe. TE LO VAS A CARGAR!!");
		    	            System.out.println("Anda, empieza de nuevo. La clave duplicada es "+clave);
		    	            System.out.println("Adiós");
		    	            break;
		    	        }
	...
	private static void conecta() throws Exception {		
		String cadenaConexion = "jdbc:oracle:thin:@vmcgaeap004.cloud.es.deloitte.com:1521:CGAE";
		String usuario = "uscgae";
		String clave = "uscgae";
	...
	private static void insertGenRecursos(StringBuffer trace, String clave, String valor) {
	...
	private static void updateGenRecursos(StringBuffer trace, String clave, String valor) {
