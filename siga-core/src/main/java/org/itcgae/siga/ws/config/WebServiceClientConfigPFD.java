...
public class WebServiceClientConfigPFD extends WsConfigurerAdapter {
    ...
    @Bean
    public PFDClient webServiceTemplatePDF() throws SOAPException, KeyManagementException, NoSuchAlgorithmException {
    	PFDClient client = new PFDClient();
    	WebServiceTemplate webServiceTemplate = new WebServiceTemplate();
		...
		webServiceTemplate.setDefaultUri("http://10.60.3.117:7019/plataformafirma2/PFDService");
		webServiceTemplate.setMarshaller(new XmlBeansMarshaller());
    	webServiceTemplate.setUnmarshaller(new XmlBeansMarshaller());
