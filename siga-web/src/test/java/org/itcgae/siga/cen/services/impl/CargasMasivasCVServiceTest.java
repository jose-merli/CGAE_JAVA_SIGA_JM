...
public class CargasMasivasCVServiceTest {
	...
	@Test
	public void createExcelFile() throws BusinessException {
	...
	public void uploadFile() throws Exception {
		...
		when(genRecursosCatalogosMapper.selectByPrimaryKey(Mockito.any(GenRecursosCatalogosKey.class))).thenReturn(genRecursosCatalogos);
		
		when(cenTiposcvsubtipo1Mapper.selectByExample(Mockito.any(CenTiposcvsubtipo1Example.class))).thenReturn(tiposcvsubtipo1s);
		
		when(cenTiposcvsubtipo1Mapper.selectByExample(Mockito.any(CenTiposcvsubtipo1Example.class))).thenReturn(tiposcvsubtipo1s);

		when(genRecursosCatalogosMapper.selectByPrimaryKey(Mockito.any(GenRecursosCatalogosKey.class))).thenReturn(genRecursosCatalogos);
		
		when(cenTiposcvsubtipo2Mapper.selectByExample(Mockito.any(CenTiposcvsubtipo2Example.class))).thenReturn(tiposcvsubtipo2);

		when(genRecursosCatalogosMapper.selectByPrimaryKey(Mockito.any(GenRecursosCatalogosKey.class))).thenReturn(genRecursosCatalogos);

		when(cenDatoscvMapperExtends
				.selectByPrimaryKey(Mockito.any(CenDatoscvKey.class))).thenReturn(cenDatoscv);
	...
	//	MockMultipartHttpServletRequest mockreq = testUtils.getMultipartHttpServletRequest();
...
//		assertThat(updateResponseDTOResultado).isEqualTo(updateResponseDTOEsperado);
	...
	@Test
	public void searchCV() throws Exception {
		...
		when(cenCargaMasivaExtendsMapper.selectEtiquetas(idInstitucion, cargaMasivaItem)).thenReturn(cargaMasivaItemList);
	...
	public void downloadOriginalFileTest()  throws Exception{
		...
		ResponseEntity<InputStreamResource> response = cargasMasivasCVServiceImpl.downloadOriginalFile(cargaMasivaIteem, mockreq);
		assertThat(response).isNotNull();
	}
	...
	public void downloadLogFileTest() throws Exception{
		...
		ResponseEntity<InputStreamResource> response = cargasMasivasCVServiceImpl.downloadLogFile(cargaMasivaIteem, mockreq);
		assertThat(response).isNotNull();
	}
}
