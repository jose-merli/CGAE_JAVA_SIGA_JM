...
public class CombosGeneralesController {
	...
	@GetMapping("/turnosAsuntos")
	public ResponseEntity<ComboDTO> comboTurnosAsuntos(HttpServletRequest request, String idTipo) {
		ComboDTO response = comboService.comboTurnosAsuntos(request, idTipo);
		return new ResponseEntity<ComboDTO>(response, HttpStatus.OK);
	}
