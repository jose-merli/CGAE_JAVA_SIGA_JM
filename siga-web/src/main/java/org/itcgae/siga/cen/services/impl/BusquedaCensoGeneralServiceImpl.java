				...
				if (!UtilidadesString.esCadenaVacia(busquedaPerFisicaSearchDTO.getNif())) {
						...
						if (null != colegiado.getColegiacionArray() && colegiado.getColegiacionArray().length > 0) {
							for (ColegiacionType colegiadoColegiacion : colegiado.getColegiacionArray()) {
								...
								if (!UtilidadesString.esCadenaVacia(busquedaPerFisicaSearchDTO.getNumeroColegiado())) {
									if (null != colegiado.getColegiacionArray()
											&& colegiado.getColegiacionArray().length > 0) {
										encontrado = Boolean.FALSE;
										if (busquedaPerFisicaSearchDTO.getNumeroColegiado()
												.equals(colegiadoColegiacion.getNumColegiado())) {
									...
									for (int j = 0; j < busquedaPerFisicaSearchDTO.getIdInstitucion().length; j++) {
										List<CenInstitucion> instituciones = institucionesService
												.getidInstitucionByCodExterno(
														colegiadoColegiacion.getColegio().getCodigoColegio());
										if (busquedaPerFisicaSearchDTO.getIdInstitucion()[j]
												.equals(String.valueOf(instituciones.get(0).getIdinstitucion()))) {
											encontrado = Boolean.TRUE;
										}
									}
								}
								...
								if (encontrado) {
									...
									if (null != colegiado.getDatosPersonales().getApellido1()) {
										busquedaPerFisica
												.setPrimerApellido(colegiado.getDatosPersonales().getApellido1());
									} else {
										busquedaPerFisica.setPrimerApellido("");
									}
									if (null != colegiado.getDatosPersonales().getApellido2()) {
										busquedaPerFisica
												.setSegundoApellido(colegiado.getDatosPersonales().getApellido2());
									} else {
									...
									switch (colegiadoColegiacion.getSituacion().getSituacionEjerProfesional()
											.toString()) {
									case SigaConstants.CENSO_WS_SITUACION_BAJACOLEGIO:
										recursosExample.createCriteria()
												.andIdrecursoEqualTo("censo.ws.situacionejerciente.BAJA_COLEGIAL")
												.andIdlenguajeEqualTo(idLenguaje);
									...
									case SigaConstants.CENSO_WS_SITUACION_EJERCIENTE:
										...
										recursosExample.createCriteria()
												.andIdrecursoEqualTo("censo.ws.situacionejerciente.EJERCIENTE")
												.andIdlenguajeEqualTo(idLenguaje);
									...
									case SigaConstants.CENSO_WS_SITUACION_INSCRITO:
										...
										recursosExample.createCriteria()
												.andIdrecursoEqualTo("censo.ws.situacionejerciente.INSCRITO")
												.andIdlenguajeEqualTo(idLenguaje);
										recurso = genRecursosMapper.selectByExample(recursosExample);
										busquedaPerFisica.setSituacion(recurso.get(0).getDescripcion());
										break;
									case SigaConstants.CENSO_WS_SITUACION_NOEJERCIENTE:
										...
										recursosExample.createCriteria()
												.andIdrecursoEqualTo("censo.ws.situacionejerciente.NO_EJERCIENTE")
												.andIdlenguajeEqualTo(idLenguaje);
										recurso = genRecursosMapper.selectByExample(recursosExample);
										busquedaPerFisica.setSituacion(recurso.get(0).getDescripcion());
										break;
									...
									String fechaEstado = format1
											.format(colegiadoColegiacion.getSituacion().getFechaSituacion().getTime());
									...
									List<CenPersona> cenPersona = cenPersonaExtendsMapper
											.selectByExample(cenPersonaExample);
					...
					if (!UtilidadesString.esCadenaVacia(busquedaPerFisicaSearchDTO.getNumeroColegiado())
						...
						for (String id : busquedaPerFisicaSearchDTO.getIdInstitucion()) {
							...
							busquedaColegiadoResponseDocument = clientCENSO.busquedaColegiadoSinIdentificacion(
									colegiadoRequestDocument, config.get(0).getValor());
							...
							for (int i = 0; i < colegiadoInstitucionName.length; i++) {
								colegiadosName.add(
										(com.colegiados.info.redabogacia.BusquedaColegiadoResponseDocument.BusquedaColegiadoResponse.Colegiado) colegiadoInstitucionName[i]);
							}
						}

						colegiadoName = colegiadosName.toArray(
								new com.colegiados.info.redabogacia.BusquedaColegiadoResponseDocument.BusquedaColegiadoResponse.Colegiado[0]);
					...
					if (null != colegiadoName && colegiadoName.length > 0) {
						...
						if (null != busquedaPerFisicaSearchDTO.getIdInstitucion()
							...
							for (int i = 0; i < busquedaPerFisicaSearchDTO.getIdInstitucion().length; i++) {
								List<CenInstitucion> instituciones = institucionesService
										.getCodExternoByidInstitucion(busquedaPerFisicaSearchDTO.getIdInstitucion()[i]);
						...
						for (int i = 0; i < colegiadoName.length; i++) {
							...
							if (null != colegiadoName[i].getColegiacionArray()
									&& colegiadoName[i].getColegiacionArray().length > 0) {
								for (ColegiacionType colegiadoName2 : colegiadoName[i].getColegiacionArray()) {
									...
									if (null != colegiadoName[i].getDatosPersonales().getApellido1()) {
										busquedaPerFisica.setPrimerApellido(
												colegiadoName[i].getDatosPersonales().getApellido1());
									} else {
										busquedaPerFisica.setPrimerApellido("");
									}
									if (null != colegiadoName[i].getDatosPersonales().getApellido2()) {
										busquedaPerFisica.setSegundoApellido(
												colegiadoName[i].getDatosPersonales().getApellido2());
									} else {
										busquedaPerFisica.setSegundoApellido("");
									}
									...
									if (colegiadoName[i].getDatosPersonales().getIdentificacion().getNIF() != null) {
										busquedaPerFisica.setNif(
												colegiadoName[i].getDatosPersonales().getIdentificacion().getNIF());

									} else if (colegiadoName[i].getDatosPersonales().getIdentificacion()
											.getPasaporte() != null) {
										busquedaPerFisica.setNif(colegiadoName[i].getDatosPersonales()
												.getIdentificacion().getPasaporte());
									}

									if (busquedaPerFisica.getNif() != null) {
										// Extraemos el idPersona
										CenPersonaExample cenPersonaExample = new CenPersonaExample();
										cenPersonaExample.createCriteria().andNifcifEqualTo(busquedaPerFisica.getNif());
										List<CenPersona> cenPersona = cenPersonaExtendsMapper
												.selectByExample(cenPersonaExample);
									...
									if (null != colegiadoName2.getSituacion()) {
										if (null != colegiadoName2.getSituacion().getSituacionEjerProfesional()) {
											GenRecursosExample recursosExample = new GenRecursosExample();
											switch (colegiadoName2.getSituacion().getSituacionEjerProfesional()
													.toString()) {
											case SigaConstants.CENSO_WS_SITUACION_BAJACOLEGIO:
												recursosExample.createCriteria()
														.andIdrecursoEqualTo(
																"censo.ws.situacionejerciente.BAJA_COLEGIAL")
														.andIdlenguajeEqualTo(idLenguaje);
												List<GenRecursos> recurso = genRecursosMapper
														.selectByExample(recursosExample);
												busquedaPerFisica.setSituacion(recurso.get(0).getDescripcion());
												break;
											case SigaConstants.CENSO_WS_SITUACION_EJERCIENTE:
											...
											case SigaConstants.CENSO_WS_SITUACION_NOEJERCIENTE:
												...
												recursosExample.createCriteria()
														.andIdrecursoEqualTo(
																"censo.ws.situacionejerciente.NO_EJERCIENTE")
														.andIdlenguajeEqualTo(idLenguaje);
												recurso = genRecursosMapper.selectByExample(recursosExample);
												busquedaPerFisica.setSituacion(recurso.get(0).getDescripcion());
												break;
											...
											SimpleDateFormat format1 = new SimpleDateFormat("dd-MM-yyyy");
											String fechaEstado = format1.format(
													colegiadoName2.getSituacion().getFechaSituacion().getTime());
											busquedaPerFisica.setFechaEstado(fechaEstado);
										}
									}
									...
									if (null != colegiadoName2.getColegio()) {
										List<CenInstitucion> instituciones = institucionesService
												.getidInstitucionByCodExterno(
														colegiadoName2.getColegio().getCodigoColegio());
										if (null != instituciones && instituciones.size() > 0) {
											busquedaPerFisica.setColegio(instituciones.get(0).getAbreviatura());
											busquedaPerFisica.setNumeroInstitucion(
													instituciones.get(0).getIdinstitucion().toString());
									...
									if (null != colegiadoName[i].getLocalizacion()) {
										busquedaPerFisica
												.setDomicilio(colegiadoName[i].getLocalizacion().getDomicilio());
