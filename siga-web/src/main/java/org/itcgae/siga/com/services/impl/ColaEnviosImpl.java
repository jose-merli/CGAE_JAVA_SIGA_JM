...
public class ColaEnviosImpl implements IColaEnvios {
	...
	// @Transactional
	@Scheduled(cron = "${cron.pattern.scheduled.Envios: 0 0/5 * * * ?}")
	@Override
	public void execute() {
	...
	public void preparaCorreo(EnvEnvios envio) throws Exception {
		...
		if (envioMasivo) {
			...
			if (envio.getIdtipoenvios().toString().equals(SigaConstants.ID_ENVIO_MAIL)) {
				idEstadoEnvio = _enviosService.envioMail(String.valueOf(envio.getIdinstitucion()),
						String.valueOf(envio.getIdenvio()), remitentedto, destinatarios, asuntoFinal, cuerpoFinal,
						documentosEnvio, envioMasivo);
			} else {
			...
			exampleDest.createCriteria().andIdenvioEqualTo(envio.getIdenvio())
			...
			if (destinatariosEntities != null && destinatariosEntities.size() > 0) {
				...
				// Generamos los informes para adjuntarlos al envio
				List<DatosDocumentoItem> documentosEnvio = _dialogoComunicacionService
						.generarDocumentosEnvio(envio.getIdinstitucion().toString(), envio.getIdenvio().toString());

				LOGGER.debug("Procedemos al envio del email: tipo " + envio.getIdtipoenvios() + "--"
						+ envio.getIdtipoenvios().toString().equals(SigaConstants.ID_ENVIO_MAIL));
				...
				if (envio.getIdtipoenvios().toString().equals(SigaConstants.ID_ENVIO_MAIL)) {
					...
					if (envio.getIdtipoenvios().toString().equals(SigaConstants.ID_ENVIO_DOCUMENTACION_LETRADO)) {
						idEstadoEnvio = _enviosService.envioMail(String.valueOf(envio.getIdinstitucion()),
								String.valueOf(envio.getIdenvio()), remitentedto, destinatarios, asuntoFinal,
								cuerpoFinal, null, envioMasivo);
					}
					// Añadimos los informes al envio para que puedan ser descargados.
//					for (DatosDocumentoItem datosDocumentoItem : documentosEnvio) {
//						EnvDocumentos documento = new EnvDocumentos();
...
//					}
	...
	private String getAsuntoCuerpoFinal(EnvEnvios envio, List<Map<String, Object>> resultadosConsultas, String asunto,
			String idCampoAsunto) {
		if (asunto == null) {
			asunto = "";
		}
	...
	private void addDocumentosAdjuntos(EnvEnvios envio, List<DatosDocumentoItem> documentosEnvio) {
		...
		exampleDoc.createCriteria().andIdenvioEqualTo(envio.getIdenvio())
				.andIdinstitucionEqualTo(envio.getIdinstitucion());
		...
		if (documentos != null) {
			for (EnvDocumentos documento : documentos) {
				...
				String path = _enviosMasivosService.getPathFicheroEnvioMasivo(envio.getIdinstitucion(),
	...
	private void addResultadoConsultaDatosPlantillaEnvio(EnvEnvios envio,
			List<Map<String, Object>> resultadosConsultas) {
		...
		exampleConsulta.createCriteria().andIdinstitucionEqualTo(envio.getIdinstitucion())
				.andIdenvioEqualTo(envio.getIdenvio()).andFechabajaIsNull().andIdobjetivoEqualTo(objetivoDatos);

		List<EnvConsultasenvio> consultasDatosPlantilla = _envConsultasenvioMapper
				.selectByExampleWithBLOBs(exampleConsulta);
	...
	private void addDestintatariosEtiquetas(EnvEnvios envio, List<DestinatarioItem> destinatarios, boolean isBuroSMS,
			List<EnvDestinatariosBurosms> listEnvDestinatariosBurosms) {
		...
		for (EnvEnviosgrupocliente envEnviosgrupocliente : etiquetasEnvio) {
			...
			etiqueta.createCriteria().andIdgrupoEqualTo(envEnviosgrupocliente.getIdgrupo())
					.andIdinstitucionEqualTo(envio.getIdinstitucion());
			...
			for (CenGruposclienteCliente persona : personas) {
				...
				if (SigaConstants.ID_ENVIO_SMS.equals(envio.getIdtipoenvios().toString())
						|| SigaConstants.ID_ENVIO_BURO_SMS.equals(envio.getIdtipoenvios().toString())) {
				...
				if (direcciones != null && direcciones.size() > 0) {
					...
					for (CenDirecciones dir : direcciones) {
						...
						if ((SigaConstants.ID_ENVIO_MAIL.equals(envio.getIdtipoenvios().toString())
								|| SigaConstants.ID_ENVIO_DOCUMENTACION_LETRADO
										.equals(envio.getIdtipoenvios().toString()))
								&& dir.getCorreoelectronico() != null) {
							if (!añadido) {
...
//								LOGGER.info("PRUEBA ENVIOS ETIQUETAS : " + destinatarios.toString());
						...
						if ((SigaConstants.ID_ENVIO_SMS.equals(envio.getIdtipoenvios().toString())
							...
							if (!añadido) {
								destinatario = new DestinatarioItem();
								añadido = true;
								addDestBuroSMS(isBuroSMS, listEnvDestinatariosBurosms, envio.getIdenvio(),
										dir.getIdinstitucion(), dir.getIdpersona(), dir.getMovil());
							}
						}
	...
	protected void insertaEnvDestinatarios(EnvEnvios etiquetasDTO, CenDirecciones direccion,
			CenGruposclienteCliente cenPersona) {
		...
		try {
			...
			if (direccion.getCorreoelectronico() == null) {
				ejemplo.createCriteria().andIdinstitucionEqualTo(cenPersona.getIdinstitucion())
						.andIdenvioEqualTo(etiquetasDTO.getIdenvio()).andIdpersonaEqualTo(cenPersona.getIdpersona());
			} else {
				ejemplo.createCriteria().andIdinstitucionEqualTo(cenPersona.getIdinstitucion())
						.andIdenvioEqualTo(etiquetasDTO.getIdenvio()).andIdpersonaEqualTo(cenPersona.getIdpersona())
			...
			// LOGGER.info("PRUEBA ENVIOS ETIQUETAS : " + cenPersona.getNifcif() + " IDGRUPO
			// = " + etiquetasDTO.getEtiquetasSeleccionadas()[i].getValue());
			if (destinatariosExistentes.size() > 0) {

				CenPersona personaTag = _cenPersonaMapper.selectByPrimaryKey(cenPersona.getIdpersona());
	...
	private void addDestinatariosConsultas(EnvEnvios envio, List<DestinatarioItem> destinatarios, boolean isBuroSMS,
			List<EnvDestinatariosBurosms> listEnvDestinatariosBurosms) {

		List<ConsultaItem> consultaItem = _envDestConsultaEnvioExtendsMapper
				.selectConsultasDestEnvio(envio.getIdinstitucion(), envio.getIdenvio().toString());
			...
			for (ConsultaItem consulta : consultaItem) {
				...
				if (result != null && result.size() > 0) {
					...
					for (Map<String, Object> map : result) {
						...
						addDestBuroSMS(isBuroSMS, listEnvDestinatariosBurosms, envio.getIdenvio(),
								envio.getIdinstitucion(), idPersona, destinatario.getMovil());

						// Obtenemos la persona
						CenPersona cenPersona = _cenPersonaMapper
								.selectByPrimaryKey(Long.parseLong(String.valueOf(map.get("IDPERSONA"))));

						// Creamos la persona con CenGruposclienteCliente que es como lo va a uar
						// insertaEnvDestinatarios().
						...
						// Obtenemos la direccion preferente de la persona
						exampleDir.createCriteria()
								.andIdpersonaEqualTo(Long.parseLong(String.valueOf(map.get("IDPERSONA"))))
								.andIdinstitucionEqualTo(Short.parseShort(String.valueOf(map.get("IDINSTITUCION"))))
								.andFechabajaIsNull()
								.andPreferenteLike("%" + SigaConstants.TIPO_PREFERENTE_CORREOELECTRONICO + "%");
						List<CenDirecciones> direcciones = _cenDireccionesMapper.selectByExample(exampleDir);
						if (direcciones.size() > 0)
							insertaEnvDestinatarios(envio, direcciones.get(0), persona);
					}
				}
			}
		}
	...
	private void addDestinatariosIndividuales(Short idinstitucion, Long idenvio, List<DestinatarioItem> destinatarios,
			boolean isBuroSMS, List<EnvDestinatariosBurosms> listEnvDestinatariosBurosms) {
		if (idenvio != null && idinstitucion != null) {
			LOGGER.debug("Buscamos los destinatarios individuales para el idinstitucion " + idinstitucion
					+ " e idenvio = " + idenvio);
			...
			if (destIndv != null) {
				for (EnvDestinatarios destinatario : destIndv) {
					...
					addDestBuroSMS(isBuroSMS, listEnvDestinatariosBurosms, idenvio, idinstitucion,
							destinatario.getIdpersona(), destinatario.getMovil());
				}
			}
		}
	}
	...
	public void preparaEnvioSMS(EnvEnvios envio, boolean isBuroSMS) {
		...
		if (envioMasivo) {
			...
			// Obtenemos destinatarios individuales.
			addDestinatariosIndividuales(envio.getIdinstitucion(), envio.getIdenvio(), destinatarios, isBuroSMS,
					listEnvDestinatariosBurosms);
			// obtenemos los destinatarios por consultas de destinatarios
			...
			if (destinatariosEntities != null && destinatariosEntities.size() > 0) {
				...
				for (EnvDestinatarios dest : destinatariosEntities) {
					...
					addDestBuroSMS(isBuroSMS, listEnvDestinatariosBurosms, envio.getIdenvio(), envio.getIdinstitucion(),
							dest.getIdpersona(), dest.getMovil());
				}
...
//		returnDestinatario.setCorreoElectronico(envDestinatariosBurosms.getCorreoelectronico());
	...
	private void addDestBuroSMS(boolean isBuroSMS, List<EnvDestinatariosBurosms> listEnvDestinatariosBurosms,
			Long idenvio, Short idinstitucion, Long idpersona, String numMovil) {
	...
	private DestinatarioItem obtenerDestinatario(Map<String, Object> registro, String idPersonaAlias,
			String correoElectronicoAlias, String movilAlias, String domicilioAlias) {
